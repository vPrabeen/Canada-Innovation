...
<footer class="footer">
	 <div class="span_6">
       <div class="grid-container">
         <div class="get">
          <h2>Get in Touch</h2>
           <?php echo do_shortcode( '[contact-form-7 id="55" title="Get in Touch form"]' ); ?>
         </div>
       </div>
     </div>
...
<script type="text/javascript">
    ...
    $('.main-content').each(function(){  
      var highestBox = 0;
      $('.blogbox', this).each(function(){
        if($(this).height() > highestBox) {
          highestBox = $(this).height(); 
        }
      });  

      $('.blogbox',this).height(highestBox);
    }); 
});
...
$(window).load(function(){
  equalheight('.wprev_preview_tcolor1_T1');
  equalheight('.blogbox');
  equalheight('.blogbox h3');
...
jQuery(document).ready(function(){
      ...
      //Accordion Nav
      jQuery('.overlay-content').navAccordion({
        expandButtonText: '<i class="fa fa-arrow-down"></i>',  //Text inside of buttons can be HTML
        collapseButtonText: '<i class="fa fa-arrow-up"></i>'
      }, 
      function(){
...
<script type="text/javascript">
  ...
  //** Calculte Mortgage on Page Load **//
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if(res.status=="success"){   
        ...
        $("#display_payment").text(res.display_payment);
        $("#display_mortgage_amount").text(res.display_mortgage_amount);
        $("#display_payment_term").text(res.display_payment_term);
        $("#down_pmr").attr("href", res.mortgage_report_path);
      }
  ...
  $('#type_of_mortgage').on('change', function(){ 
    var rate;
    var mortgage_type = $(this).val();
  ...
  if($("#starting_mortgage_year").val()==""){ 
    $("#starting_mortgage_year").val('0');
    $("#year_str").html('year'); 
  }
  $("#starting_mortgage_year").on('keyup', function(){
    if($(this).val()>1){ $("#year_str").html('years'); }
    else{  $("#year_str").html('year'); }
  });
  
  //** Month Input String Change **//
  if($("#starting_mortgage_month").val()==""){ 
    $("#starting_mortgage_month").val('0');
    $("#month_str").html('month'); 
  } 
  $("#starting_mortgage_month").on('keyup', function(){
    if($(this).val()>1){ 
      $("#month_str").html('months');
    } else if($(this).val()==0){ 
      $("#month_str").html('month'); 
    } else if($(this).val()==""){  
      $(this).val('0');
...
$(document).ready(function(){ // Start Document Ready
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if(res.status=="success"){   
        //** Pass the data to the Result Section **//
        $("#display_payment").html(res.display_payment);
        $("#display_mortgage_amount").html(res.display_mortgage_amount);
        $("#display_payment_term").html(res.display_payment_term);        
        $("#down_pmr").attr("href", res.mortgage_report_path);
        
        if(res.display_payment_term_boost!=""){
          $("#mortgage_term_booster").removeClass("invisible");
          $("#mortgage_term_booster").html('<p class="label label-success"><abbr>'+res.display_payment_term_boost+'</abbr></p>').fadeIn(1500);
                    
        } else {
          $("#mortgage_term_booster").addClass("invisible");
          $("#mortgage_term_booster").html('');
        } 
      }
  ...
  //** Number Validation  **//
  $(".calcV").on("keypress keyup blur paste",function (event) { // 
  ...
  $(".calcFV").on("keypress keyup blur paste",function (event) { //keypress keyup 
  ...
  $(".calcM").on("keyup keypress paste input",function (event) {
    var ip_val  = $(this).val();
    if (ip_val.indexOf(',') > -1) { 
      $("#comma_err p").html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> A special character comma has been detected. Comma has been removed from your recently given input value to make consistent the realtime calculation.');
      $("#comma_err").removeClass('invisible');
      
      $("#comma_err").fadeIn('slow');
      setTimeout(function(){  
        $("#comma_err p").html('');
        $("#comma_err").addClass('invisible');
      }, 5000);
    } 
  }); 
  ...
  $(".calcM").on("keyup keypress blur paste click input ",function (event) {
    
    var keycode = event.keyCode || event.which;
    if (keycode === 9) {  
      input_id  = $(this).attr('id');
      input_val   = $(this).val().replace(/,/g, '');      
    } else {
      input_id  = $(this).attr('id');
      input_val   = $(this).val().replace(/,/g, '');
    }
    ...
    if( input_id=="mortgage_amount" && ( input_val=="" || input_val!="" || isNaN(input_val) ) ){ 
      ...
      if(input_val!=""){ // If Input Field is Not Blank or Blanked by useing Backspace
        ...
        if(input_val < minMA_val){ // If Inputed value is less then Minimum value
          ...
          $("#"+input_id).val(minMA_val);
        
          $("#mamt_err p").html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> Your value has been reset to minimum of '+minMA_val+'.');
          $("#mamt_err").removeClass('invisible');
          
          $("#mamt_err").fadeIn(4000);
          setTimeout(function(){  
            $("#mamt_err p").html('');
            $("#mamt_err").addClass('invisible');
          }, 3000);
          
        } else if(input_val > maxMA_val){ // If Inputed value is greater then Maximum value
          
          $("#"+input_id).val(maxMA_val);
          $("#mamt_err p").html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> Your value has been reset to maximum of '+maxMA_val+'.');
          $("#mamt_err").removeClass('invisible');
    ...
    if( input_id=="starting_mortgage_year" && ( input_val=="" || input_val!="" || isNaN(input_val) ) ){  
      ...
      if(input_val!=""){ // If Input Field is Not Blank or Blanked by useing Backspace
        ...
        if(input_val < minYr_val){ // If Inputed value is less then Minimum value
          ...
          $("#"+input_id).val(minYr_val);
        
          $("#mynm_err p").html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> Your value has been reset to minimum of '+minYr_val+'.');
          $("#mynm_err").removeClass('invisible');
          
          $("#mynm_err").fadeIn(4000);
          setTimeout(function(){  
            $("#mynm_err p").html('');
            $("#mynm_err").addClass('invisible');
          }, 3000);
          
        } else if(input_val > maxYr_val){ // If Inputed value is greater then Maximum value
          
          $("#"+input_id).val(maxYr_val);
          $("#mynm_err p").html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> Your value has been reset to maximum of '+maxYr_val+'.');
          $("#mynm_err").removeClass('invisible');
          ...
          if(roff_val==maxYr_val){ // If Mortgage Year is Max then Month will be 0 and Disable Input
            $("#starting_mortgage_month").val(0);
            $("#starting_mortgage_month").prop( "disabled", true );
          } else { // Reset disable field
            $("#starting_mortgage_month").prop( "disabled", false );
          }
    ...
    if( input_id=="starting_mortgage_month" && ( input_val=="" || input_val!="" || isNaN(input_val) ) ){  
      ...
      if(input_val!=""){ // If Input Field is Not Blank or Blanked by useing Backspace
        ...
        if(input_val < minMon_val){ // If Inputed value is less then Minimum value
          ...
          $("#"+input_id).val(minMon_val);
        
        } else if(input_val > maxMon_val){ // If Inputed value is greater then Maximum value
          
          $("#"+input_id).val(maxMon_val);
          $("#mynm_err p").html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> Your value has been reset to maximum of '+maxMon_val+'.');
          $("#mynm_err").removeClass('invisible');
    ...
    if( input_id=="interest_rate" && ( input_val=="" || input_val!="" || isNaN(input_val) ) ){ 
      ...
      if(input_val!=""){ // If Input Field is Not Blank or Blanked by useing Backspace
        ...
        if(input_val < minINT_val){ // If Inputed value is less then Minimum value
          ...
          $("#int_err p").html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> Your value has been reset to minimum of '+minINT_val+'.');
          $("#int_err").removeClass('invisible');
          
          $("#int_err").fadeIn(4000);
          setTimeout(function(){  
            $("#int_err p").html('');
            $("#int_err").addClass('invisible');
          }, 3000);
          
        } else if(input_val > maxINT_val){ // If Inputed value is greater then Maximum value
          
          $("#"+input_id).val(maxINT_val);
          $("#int_err p").html('<i class="fa fa-exclamation-circle" aria-hidden="true"></i> Your value has been reset to maximum of '+maxINT_val+'.');
          $("#int_err").removeClass('invisible');
