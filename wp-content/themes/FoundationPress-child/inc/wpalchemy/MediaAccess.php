	...
	public function getIdField(array $attr)
		{
			...
			###
			$thumb = wp_get_attachment_image_src( $attr['value'], 'thumbnail');
			if($thumb)
			{
				$image = '<img src="'.$thumb[0].'" width="60" />';
			}
			return $image.'<input ' . implode(' ', $elem_attr) . '/>';
		}
	/**
					...
					if (typeof send_to_editor === 'function')
					{
						...
						send_to_editor = function(html)
						{
							...
							if (wpalchemy_mediafield)
							{
								...
								if(wpalchemy_mediaidfield.prev('img').length > 0 ) {
									wpalchemy_mediaidfield.prev('img').attr('src', url);
									console.log('got it');
								}
								else {
									console.log('not got it');
									wpalchemy_mediaidfield.before('<img src="'+url+'" width="60" />');
								}
